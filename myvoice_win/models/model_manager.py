"""Model cache utilities (Windows build).
...
Windows equivalent of ``myvoice.models.model_manager``: identical behavior,
the only difference is ``models_cache_dir`` comes from ``myvoice_win.paths``
(Windows %LOCALAPPDATA%-based) instead of ``myvoice.paths`` (Linux
XDG-based).
"""
